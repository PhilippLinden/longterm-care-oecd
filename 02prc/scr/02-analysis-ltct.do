...
eststo sumstats: estpost summarize ///
		...
		zr_choice_index ///
		means_any
	esttab sumstats using "$fig\20200506_LTCT_PL_Descriptive_Statistics.rtf", replace cell("mean sd min max") mtitle("Descriptives") label
...
save "$pdta\LTCT_aux_single.dta", replace
...
graph export "$fig\screeplots_R_G.tif", replace

save "$pdta\LTCT_aux_k_9.dta", replace
...
graph export "$fig\screeplots_Z_G.tif", replace

save "$pdta\LTCT_aux_k_10.dta", replace
...
graph export "$fig\screeplots_R_E2.tif", replace

save "$pdta\LTCT_aux_k_11.dta", replace
...
graph export "$fig\screeplots_Z_E2.tif", replace

save "$pdta\LTCT_aux_k_12.dta", replace
...
***Merge of single pdtasets

clear all
use "$pdta\LTCT_aux_single.dta"
	merge 1:1 id using "$pdta\LTCT_aux_k_9.dta"
		drop _merge
	merge 1:1 id using "$pdta\LTCT_aux_k_10.dta"
		drop _merge
	merge 1:1 id using "$pdta\LTCT_aux_k_11.dta"
		drop _merge
	merge 1:1 id using "$pdta\LTCT_aux_k_12.dta"
		drop _merge
...
compress
save "$pdta\LTCT_Final.dta", replace
lab data "Long-term-care Typology_Final"
		
erase "$pdta\LTCT_aux_k_9.dta"
erase "$pdta\LTCT_aux_k_10.dta"
erase "$pdta\LTCT_aux_k_11.dta"
erase "$pdta\LTCT_aux_k_12.dta"
erase "$pdta\LTCT_aux_single.dta"
...
log using "$text\20200507_LTCT_PL_Cluster_Country.log", replace
...
cluster dend _clus_1, lab(id_leaf) ///
	...
	graph export "$fig\01_Dendrogramm_R_G_A.tif", replace	
...
cluster dend _clus_2, lab(id_leaf) ///
	...
	graph export "$fig\02_Dendrogramm_Z_G_A.tif", replace
...
cluster dend _clus_3, lab(id_leaf) ///
	...
	graph export "$fig\03_Dendrogramm_R_E2_A.tif", replace	
...
cluster dend _clus_4, lab(id_leaf) ///
	...
	graph export "$fig\04_Dendrogramm_Z_E2_A.tif", replace
...
cluster dend _clus_5, lab(id_leaf) ///
	...
	graph export "$fig\05_Dendrogramm_R_G_W.tif", replace	
...
cluster dend _clus_6, lab(id_leaf) ///
	...
	graph export "$fig\06_Dendrogramm_Z_G_W.tif", replace
...
cluster dend _clus_7, lab(id_leaf) ///
	...
	graph export "$fig\07_Dendrogramm_R_E2_W.tif", replace	
...
cluster dend _clus_8, lab(id_leaf) ///
	...
	graph export "$fig\08_Dendrogramm_Z_E2_W.tif", replace
